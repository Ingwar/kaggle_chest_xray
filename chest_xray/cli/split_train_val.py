...
def split_metadata(original_metadata: pd.DataFrame, train_fraction: float) -> Tuple[pd.DataFrame, pd.DataFrame]:
    image_ids = pd.unique(original_metadata['image_id'])
    train_image_ids, val_image_ids = train_test_split(image_ids, train_size=train_fraction)
...
def main() -> None:
    ...
    train_metadata, validation_metadata = split_metadata(original_metadata, args.train_fraction)
